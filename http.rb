...
puts_error_message('"slack" or "mac"を入力して下さい') if (notification_type =~ /\A(slack|mac)+\z/).nil?
postal_code = ARGV[1] || '1070062'
want_price = ARGV[2] || 1000
puts_error_message('希望配送手数料は数字で入力してください。') if (want_price =~ /\A[0-9]+\z/).nil?
...
puts_error_message('slackのwebhook urlが引数に設定されていません。') if slack_webhook_url.nil? && (notification_type == 'slack')
